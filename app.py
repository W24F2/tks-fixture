...
# --- PWA: serve service worker and manifest at expected root paths ---

@app.route('/sw.js')
def serve_sw():
    """Serve the PWA service worker so the app can register it at /sw.js."""
    resp = send_from_directory(
        os.path.join(app.static_folder, 'dist'), 'sw.js',
...
def serve_manifest():
    """Serve the PWA manifest generated by vite-plugin-pwa."""
    resp = send_from_directory(
        os.path.join(app.static_folder, 'dist'), 'manifest.webmanifest',
        conditional=True, mimetype='application/manifest+json',
    )
...
def serve_workbox(filename):
    """Serve workbox runtime JS generated by vite-plugin-pwa."""
    resp = send_from_directory(
        os.path.join(app.static_folder, 'dist'), filename,
        conditional=True, mimetype='application/javascript',
    )
    resp.headers['Cache-Control'] = 'public, max-age=31536000, immutable'
    return resp
